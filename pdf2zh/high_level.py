...
def translate_stream(
    ...
    **kwarg: Any,
    ...
    font_family = get_font_family(lang_out.lower())
    noto_name = NOTO_NAME
    noto = Font(noto_name, font_family[noto_name])
    font_list.append((noto_name, font_family[noto_name]))

    noto_fonts = {noto_name: noto}
    for variant in ["noto_b", "noto_i", "noto_bi"]:
        if variant in font_family and Path(font_family[variant]).is_file():
            var_font = Font(variant, font_family[variant])
            noto_fonts[variant] = var_font
            font_list.append((variant, font_family[variant]))
...
def download_remote_fonts(lang: str):
    ...
    return family.get(NOTO_NAME, family.get("noto"))


def get_font_family(lang: str) -> dict[str, str]:
    lang = lang.lower()
    LANG_NAME_MAP = {
    ...
    # Use Times New Roman family for Vietnamese (or user-specified fonts)
    if lang == "vi":
        reg_candidates = [
            os.environ.get("VI_FONT_PATH"),
            "/System/Library/Fonts/Supplemental/Times New Roman.ttf",  # macOS standard
            "/Library/Fonts/Times New Roman.ttf",  # macOS alternative
            "C:/Windows/Fonts/times.ttf",  # Windows
            "/usr/share/fonts/truetype/msttcorefonts/Times_New_Roman.ttf",  # Linux MS Core Fonts
            "/usr/share/fonts/truetype/liberation/LiberationSerif-Regular.ttf",  # Linux Liberation
        ]
        for candidate in reg_candidates:
            if candidate and Path(candidate).is_file():
                family[NOTO_NAME] = Path(candidate).as_posix()
                logger.info(f"use font for Vietnamese (regular): {candidate}")
                break

        bold_candidates = [
            os.environ.get("VI_FONT_BOLD_PATH"),
        ...
        for candidate in bold_candidates:
            if candidate and Path(candidate).is_file():
                family["noto_b"] = Path(candidate).as_posix()
                logger.info(f"use font for Vietnamese (bold): {candidate}")
                break

        italic_candidates = [
            os.environ.get("VI_FONT_ITALIC_PATH"),
        ...
        for candidate in italic_candidates:
            if candidate and Path(candidate).is_file():
                family["noto_i"] = Path(candidate).as_posix()
                logger.info(f"use font for Vietnamese (italic): {candidate}")
                break

        bolditalic_candidates = [
            os.environ.get("VI_FONT_BOLDITALIC_PATH"),
        ...
        for candidate in bolditalic_candidates:
            if candidate and Path(candidate).is_file():
                family["noto_bi"] = Path(candidate).as_posix()
                break

    if NOTO_NAME not in family:
